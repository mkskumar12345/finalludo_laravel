...
class GameChallangeApiController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if(!$ChallangeType){
            return response()->json(['status' => false, 'message' => 'Invalid Challange Type' , 'data' => []]);
        }
        ...
        $user = Auth::user();
        $user_id = $user->id;
        // $GameChallange = GameChallange::where('challenge_accepted_by',$user_id)->orWhere('challenge_created_by',$user_id)->whereIn('status',['accepted','challange_created','in_review'])->get();
        ...
        if($user->is_play == 0){
            return response()->json(['status' => false, 'message' => 'You are not allowed to play game' , 'data' => null]);
        }
        //$roomCodeData = file_get_contents('https://www.fojibhailudo.com/classicroomcode');
...

    {
        try
        {
            ...
            $ChallangeType = ChallangeType::where('slug',$request->game_type_id)->first();
            if(!$ChallangeType){
                return response()->json(['status' => true, 'message' => 'Something went wrong' , 'data' => []]);

            }

            $data['openChallanges'] = GameChallange::where('status','challange_created')->where('challenge_created_by','!=',Auth::user()->id)->where('challenge_type',$ChallangeType->id)->take(200)->orderBy('id','desc')->get()->map( function($data){
                return [
                    'id' => $data->id,
                    ...
                    'amount' => $data->amount,
                    'status' => $data->status,
                    'accepted_id' => $data->challenge_accepted_by,
            ...
            $data['challengeRunning'] = GameChallange::where('status','running')->where('challenge_type',$ChallangeType->id)->where('challenge_created_by','!=',Auth::user()->id)->where('challenge_accepted_by','!=',Auth::user()->id)->orderBy('id','desc')->take(10)->get()->map( function($data){
                 return [
                     'id' => $data->id, 
                     'c_id' => $data->challenge_created_by,
                     'cname' => $data->createBy->name??'Guest User',
            ...
            $data['myCreatedChallanges'] = GameChallange::where('status','challange_created')->where('challenge_type',$ChallangeType->id)->where('challenge_created_by',Auth::user()->id)->orderBy('id','desc')->take(10)->get()->map( function($data){
                 return [
                     'id' => $data->id,
                     'c_id' => $data->challenge_created_by,
                     'cname' => $data->createBy->name??'Guest User',
            ...
            $data['myCreatedRequestChallanges'] = GameChallange::where('status','requested')->where('challenge_type',$ChallangeType->id)->where('challenge_created_by',Auth::user()->id)->orderBy('id','desc')->take(10)->get()->map( function($data){
                 return [
                     'id' => $data->id,
                     'c_id' => $data->challenge_created_by,
                     'cname' => $data->createBy->name??'Guest User',
            ...
            $challengeinreview = GameChallange::orWhere('challenge_accepted_by',Auth::user()->id)->orWhere('challenge_created_by',Auth::user()->id)->where('challenge_type',$ChallangeType->id)->orderBy('id','desc')->take(3)->get();
            $htmlData = [];
             foreach($challengeinreview as $item){
               if($item->status=='running' || $item->status=='in_review'){
                ...
                if($item->challenge_type == $ChallangeType->id){
                    $htmlData[] = [
                        'amount' => $item->amount, 
                        'oppennt_profile' => $profilePic, 
                        'opponet_name' => $user->name, 
                        'game_slug' => $item->slug, 
                    ];
                 }
               }
             ...
             $data['myRunnungCallenge'] = $htmlData;

             /** Data*/
          
             $challengeinreview2 = GameChallange::orWhere('challenge_accepted_by',Auth::user()->id)->orWhere('challenge_created_by',Auth::user()->id)->where('challenge_type',$ChallangeType->id)->orderBy('id','desc')->take(3)->get();
             $htmlData2 = [];
              foreach($challengeinreview2 as $item){
                if($item->status=='requested'){
                 ...
                 if($item->challenge_type == $ChallangeType->id){
                     $htmlData2[] = [
                        'id' => $item->id,
                        'c_id' => $item->challenge_created_by,
                        'cname' => $item->createBy->name??'Guest User',
                        'oname' => $item->acceptedBy->name ?? 'Guest User',
                        'amount' => $item->amount,
                        'status' => $item->status,
                        'accepted_id' => $item->challenge_accepted_by,
                        'prize' => $item->winning_amount,
                     ];
                  }
                }
              }
              
              $data['myRequestedChallanges'] = $htmlData2;
    ...
    {
        $transaction = Transaction::where('user_id',Auth::user()->id)->orderBy('id','desc')->paginate(20);

        return response()->json(['status' => true, 'message' => "Succes" ,'data' => $transaction]);
    }
    public function challangeDetails(Request $request)
    {
        $validator = Validator::make($request->all(),[
            'challange_id'      => 'required|exists:game_challenge,id',
        ]);
        ...
        $data['gameChallange'] =  GameChallange::where('id',$request->challange_id)->with('challangeResult','createBy','acceptedBy')->first();
    ...
    public function cencelChallange(Request $request)
    {
        ...
        try
        {
            $challagne = GameChallange::where('id',$request->challange_id)->where('status','!=','complete')->where('status','!=','cancel')->first();
            if(empty($challagne)){
                return response()->json(['status' => false, 'message' => 'Invalid Challange ID' , 'data' => null]);
            }
